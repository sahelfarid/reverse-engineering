...
async function renderDashboardTab() {
  ...
  try {
    ...
    pane.innerHTML = `
      <div class="card-grid">
        ${statTileHtml('Connected devices', ADB_DEVICES.filter((d) => d.state === 'device').length, `${ADB_DEVICES.length} total`)}
        ${statTileHtml('Battery', b.level != null ? b.level + '%' : '—', b.charging ? 'Charging' : (b.status || ''))}
        ${statTileHtml('Storage', vol ? vol.use_pct : '—', vol ? `${escapeHtml(vol.mounted_on)} · ${formatBytes(Number(vol.available_kb) * 1024)} free` : '')}
        ${statTileHtml('Memory', memPct != null ? memPct + '%' : '—', cm.mem_available_kb ? `${formatBytes(cm.mem_available_kb * 1024)} available` : '')}
        ${statTileHtml('CPU load', cm.load_1m || '—', cm.load_5m ? `5m: ${cm.load_5m}, 15m: ${cm.load_15m}` : '')}
        ${statTileHtml('Running apps', apps.user_apps != null ? apps.user_apps : '—', apps.total_apps != null ? `${apps.total_apps} incl. system` : '')}
        ${statTileHtml('Link', device.is_wireless ? 'Wi-Fi' : 'USB', wifi.enabled == null ? '' : `Wi-Fi radio ${wifi.enabled ? 'on' : 'off'}`)}
        ${statTileHtml('Screen', screen.screen_on == null ? '—' : (screen.screen_on ? 'On' : 'Off'), '')}
        ${statTileHtml('Root', detail.device.root_available ? 'Available' : 'Unavailable', '')}
        ${statTileHtml('Foreground app', fg.package || '—', fg.activity || '')}
      </div>
    `;
  } catch (err) {
